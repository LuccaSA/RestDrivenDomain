...
namespace Rdd.Web.Tests
{
    public class WebFiltersContainerTests
    {
        ...
        public void GoodFilterShouldWork()
        {
            var filters = new HashSet<WebFilter<User>>()
            {
                new WebFilter<User>(ExpressionChain<User>.New(u => u.Name), WebFilterOperand.Equals, new List<string> { "Foo" })
            };
            var container = new WebFiltersContainer<User, int>(filters);
            Expression<Func<User, object>> expression = u => u.Name;
        ...
        public void NullFilterShouldWork()
        {
            var filters = new HashSet<WebFilter<User>>()
            {
                new WebFilter<User, string>(ExpressionChain<User>.New(u => u.Name), WebFilterOperand.Equals, null)
            };
            var container = new WebFiltersContainer<User, int>(filters);
            Expression<Func<User, object>> expression = u => u.Name;
        ...
        public void SecondLevelGoodFilterShouldWork()
        {
            var filters = new HashSet<WebFilter<User>>()
            {
                new WebFilter<User>(ExpressionChain<User>.New(u => u.MyValueObject.Name), WebFilterOperand.Equals, new List<string> { "Foo" })
            };
            var container = new WebFiltersContainer<User, int>(filters);
        ...
        public void FirstLevelFilterWithSecondLevelCheckShouldWork()
        {
            var filters = new HashSet<WebFilter<User>>()
            {
                new WebFilter<User>(ExpressionChain<User>.New(u => u.MyValueObject), WebFilterOperand.Equals, new List<MyValueObject> { null })
            };
            var container = new WebFiltersContainer<User, int>(filters);
            Expression<Func<User, object>> expression = u => u.MyValueObject.Name;
