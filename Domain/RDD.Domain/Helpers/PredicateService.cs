        ...
        where TEntity : class, IEntityBase
    {
        internal Expression<Func<TEntity, bool>> BuildBinaryExpression(FilterOperand binaryOperator, string field, object value)
        {
            ...
            if (binaryOperator == FilterOperand.Until || binaryOperator == FilterOperand.Since)
            {
                ...
                {
                    throw new FunctionalException(string.Format("Operator '{2}' only allows dates to be compared, whereas property {0} is of type {1}.", field, property.GetType().Name, binaryOperator));
                }
            }
        ...
        private bool IsQueryOnCollection(Type entityType, string field, out string collectionAccessorField, out string subField, out Type collectionType)
        {
            ...
            for (var i = 0; i < fields.Length; i++)
            {
                ...
                if (property == null)
                {
                    throw new FunctionalException(string.Format("Unknown property {0} on type {1}", member, entityType.Name));
                }

                if (!property.CanRead)
                {
                    throw new FunctionalException(string.Format("Property {0} of type {1} is set only", member, entityType.Name));
                }
