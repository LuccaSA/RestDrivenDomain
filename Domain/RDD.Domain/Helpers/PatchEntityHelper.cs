    ...
    public class PatchEntityHelper
    {
        ...
        public void PatchEntity(object entity, PostedData datas)
        {
            ...
            foreach (string key in datas.Keys)
            {
                ...
                if (property == null)
                {
                    throw new FunctionalException(string.Format("Property {0} does not exist on type {1}", key, entityType.Name));
                }
                ...
                {
                    throw new FunctionalException(string.Format("Property {0} of type {1} is not writable", property.Name, entityType.Name));
                }
                    ...
                    if (propertyType.IsSubclassOfInterface(typeof(IDictionary)))
                    {
                        ...
                        foreach (KeyValuePair<string, PostedData> sub in datas[key].Subs)
                        {
                            ...
                            if (dicValue == null && genericArguments[1].IsValueType)
                            {
                                ...
                                {
                                    throw new FunctionalException(string.Format("You cannot set a non nullable value to NULL (Property {0})", key));
                                }
                            }
